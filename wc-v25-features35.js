/* ═══════════════════════════════════════════════════════════════
   WC CORPORATE EDITOR v25.35 — 5 FEATURES
   1. Autoguardado en la nube (Firebase)
   2. Modo lectura inmersiva
   3. Buscador de imágenes sin copyright
   4. Generador de CV / Hoja de vida CR
   5. Calculadora de IVA y precios CR
   ═══════════════════════════════════════════════════════════════ */

/* ═══════════════════════════════════
   FEATURE 1: AUTOGUARDADO FIREBASE
   ═══════════════════════════════════ */

const WC_CLOUD_SAVE = {
  timer:    null,
  lastSaved: null,
  docId:    localStorage.getItem('wc-cloud-docid') || 'doc-' + Math.random().toString(36).slice(2,10),

  init() {
    localStorage.setItem('wc-cloud-docid', this.docId);
    const ed = document.getElementById('editor');
    if (!ed) return;
    ed.addEventListener('input', () => {
      clearTimeout(this.timer);
      this._updateStatus('⏳ Guardando...');
      this.timer = setTimeout(() => this.save(), 3000);
    });
    // Guardar al cerrar pestaña
    window.addEventListener('beforeunload', () => this.save());
    this._showStatus();
    showToast('☁️ Autoguardado en la nube activado');
  },

  async save() {
    const ed = document.getElementById('editor');
    if (!ed) return;
    const db = window.__wcFirebaseDB || window.firebase?.database?.();
    if (!db) {
      // Fallback a localStorage
      localStorage.setItem('wc-cloud-'+this.docId, ed.innerHTML);
      this.lastSaved = new Date();
      this._updateStatus('💾 Guardado local ' + this.lastSaved.toLocaleTimeString('es-CR'));
      return;
    }
    try {
      await db.ref('documents/' + this.docId).set({
        content:   ed.innerHTML,
        savedAt:   Date.now(),
        wordCount: ed.innerText.split(/\s+/).filter(w=>w).length
      });
      this.lastSaved = new Date();
      this._updateStatus('☁️ Guardado ' + this.lastSaved.toLocaleTimeString('es-CR'));
    } catch(e) {
      this._updateStatus('⚠️ Error al guardar');
    }
  },
  ...
  async load() {
    const db = window.__wcFirebaseDB || window.firebase?.database?.();
    if (!db) {
      const saved = localStorage.getItem('wc-cloud-'+this.docId);
      if (saved) {
        const ed = document.getElementById('editor');
        if (ed) { ed.innerHTML = saved; showToast('📂 Documento cargado desde local'); }
      }
      return;
    }
    try {
      const snap = await db.ref('documents/' + this.docId).once('value');
      const data = snap.val();
      if (data?.content) {
        const ed = document.getElementById('editor');
        if (ed) {
          ed.innerHTML = data.content;
          const date = new Date(data.savedAt).toLocaleString('es-CR');
          showToast('☁️ Documento cargado · Guardado: ' + date);
        }
      }
  ...
  _showStatus() {
    const old = document.getElementById('__cloudStatus'); if (old) return;
    const el  = document.createElement('div');
    el.id     = '__cloudStatus';
    el.style.cssText = 'position:fixed;bottom:66px;left:12px;z-index:8000;background:rgba(0,0,0,.6);color:#fff;border-radius:6px;padding:3px 10px;font-size:10px;font-family:-apple-system,sans-serif;pointer-events:none';
    el.textContent   = '☁️ Autoguardado activo';
    document.body.appendChild(el);
  },

  _updateStatus(msg) {
    const el = document.getElementById('__cloudStatus');
    if (el) el.textContent = msg;
  }
};

function openCloudSavePanel() {
  const old = document.getElementById('__cloudPanel'); if (old) old.remove();
  ...
  <div style="background:#fff;border-radius:20px;max-width:400px;width:100%;padding:22px;box-shadow:0 20px 60px rgba(0,0,0,.3)">
    ...
    <div style="font-size:12px;color:#888;margin-bottom:16px">Firebase · Sync automático</div>
    ...
    <div style="background:#f0f7ff;border-radius:10px;padding:12px;margin-bottom:14px">
      ...
      <button onclick="navigator.clipboard.writeText('${WC_CLOUD_SAVE.docId}').then(()=>showToast('ID copiado ✅'))" style="margin-top:6px;padding:4px 10px;border:1px solid #003da5;background:#fff;color:#003da5;border-radius:5px;cursor:pointer;font-size:10px">📋 Copiar ID</button>
    </div>
...
/* ═══════════════════════════════════
   FEATURE 2: MODO LECTURA INMERSIVA
   ═══════════════════════════════════ */
...
const WC_IMMERSIVE = {
  ...
  themes: {
    light:  { bg:'#fafafa', text:'#1a1a1a', overlay:'#f0f0f0' },
    sepia:  { bg:'#f4ecd8', text:'#3d2b1f', overlay:'#e8dcc8' },
    dark:   { bg:'#1a1a2e', text:'#e0e0e0', overlay:'#16213e' },
    night:  { bg:'#0d0d0d', text:'#cccccc', overlay:'#111111' },
  },

  open() {
    const ed  = document.getElementById('editor');
    if (!ed) return;
    const old = document.getElementById('__immersivePanel'); if (old) old.remove();
    const ov  = document.createElement('div');
    ov.id     = '__immersivePanel';
    const t   = this.themes[this.theme];
    ov.style.cssText = `position:fixed;inset:0;z-index:100000;background:${t.bg};overflow-y:auto;font-family:Georgia,'Times New Roman',serif;transition:background .3s`;
    ...
    ov.innerHTML = `
      ...
      <div style="position:fixed;top:0;left:0;right:0;background:${t.overlay};padding:8px 16px;display:flex;align-items:center;gap:10px;z-index:1;opacity:0;transition:opacity .3s" id="__immBar"
        ...
        <span style="font-size:11px;color:${t.text};opacity:.6">Modo lectura · ESC para salir</span>
    ...
    ov.addEventListener('scroll', () => {
      ...
      const pr  = document.getElementById('__immProgress');
    ...
    // Mostrar barra al tocar pantalla
    ov.addEventListener('touchstart', () => {
      const bar = document.getElementById('__immBar');
      if (bar) { bar.style.opacity='1'; clearTimeout(this._barTimer); this._barTimer=setTimeout(()=>{bar.style.opacity='0';},3000); }
    }, {passive:true});
  },

  _updateContent() {
    const c = document.getElementById('__immContent');
    const f = document.getElementById('__immFontSize');
    if (c) c.style.fontSize = this.fontSize + 'px';
    if (f) f.textContent   = this.fontSize + 'px';
  },

  close() {
    this.active = false;
    document.removeEventListener('keydown', this._keyHandler);
    document.getElementById('__immersivePanel')?.remove();
  }
};

function openImmersiveMode() { WC_IMMERSIVE.open(); }


/* ═══════════════════════════════════
   FEATURE 3: BUSCADOR DE IMÁGENES SIN COPYRIGHT
   ═══════════════════════════════════ */

function openFreeImages() {
  const old = document.getElementById('__imgSearchPanel'); if (old) old.remove();
  const ov  = document.createElement('div');
  ov.id     = '__imgSearchPanel';
  ...
  ov.innerHTML = `
    <div style="background:#1a2942;padding:12px 16px;display:flex;align-items:center;gap:10px;flex-shrink:0">
      <div style="font-size:15px;font-weight:700;color:#fff;flex:1">🖼️ Imágenes sin copyright</div>
      <div style="display:flex;gap:6px">
    ...
    <div style="padding:10px 16px;background:#252540;flex-shrink:0">
      <div style="display:flex;gap:6px">
        <input type="text" id="__imgQuery" placeholder="Buscar imágenes... (ej: costa rica, naturaleza, ciudad)"
        ...
        <button onclick="__imgSearch()" style="padding:10px 16px;border:none;background:#0084ff;color:#fff;border-radius:10px;cursor:pointer;font-size:14px;font-weight:600">🔍</button>
      </div>
    </div>
    <div id="__imgGrid" style="flex:1;overflow-y:auto;padding:12px;display:grid;grid-template-columns:repeat(3,1fr);gap:8px;align-content:start;background:#1a1a2e">
      <div style="grid-column:1/-1;text-align:center;color:#888;padding:40px;font-size:13px">
        Buscá imágenes libres de derechos de autor ✅<br>
    ...
    <div style="padding:8px 16px;background:#252540;font-size:10px;color:#555;text-align:center;flex-shrink:0">
      Imágenes bajo licencia Creative Commons / Dominio público · Libre para uso comercial
    </div>`;

  document.body.appendChild(ov);
  window.__imgSource = 'unsplash';
  window.__imgPage   = 1;
  document.getElementById('__imgQuery')?.focus();
}

async function __imgSearch() {
  const q    = document.getElementById('__imgQuery')?.value.trim();
  if (!q) return;
  const grid = document.getElementById('__imgGrid');
  if (grid) grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;color:#888;padding:40px;font-size:13px">⏳ Buscando imágenes...</div>';

  const src = window.__imgSource || 'unsplash';
  let images = [];

  try {
    if (src === 'unsplash') {
      const res  = await fetch(`https://api.unsplash.com/search/photos?query=${encodeURIComponent(q)}&per_page=18&client_id=TBl6sN9OmLxIh9GFkOCPF9JE9Zfsd5AecNFMHxuNUeg`);
      ...
      images = (data.results||[]).map(img => ({
        ...
        link:  img.links.html,
        desc:  img.alt_description || q,
        w:     img.width, h: img.height
      }));
    } else if (src === 'pixabay') {
      const res  = await fetch(`https://pixabay.com/api/?key=42291503-f74c4e69c5a6b72e5e8a3b8e4&q=${encodeURIComponent(q)}&image_type=photo&per_page=18&safesearch=true`);
      const data = await res.json();
      ...
      // Pexels
      const res  = await fetch(`https://api.pexels.com/v1/search?query=${encodeURIComponent(q)}&per_page=18`, {
        headers:{ 'Authorization':'a3W9JovKz7WrSqP5VDzxMhGGPbzPmkFxPLYN3sW2Bs1p3DRaHrpNdBjn' }
      });
      const data = await res.json();
    ...
    images = Array.from({length:9},(_,i)=>({
      thumb: `https://image.pollinations.ai/prompt/${encodeURIComponent(q+' '+i)}?width=300&height=200&nologo=1`,
      full:  `https://image.pollinations.ai/prompt/${encodeURIComponent(q+' photo')}?width=800&height=600&nologo=1`,
      author:'Pollinations AI',
      link:  '#',
      desc:  q + ' (generada con IA)'
    }));
  }

  if (!grid) return;
  if (!images.length) { grid.innerHTML='<div style="grid-column:1/-1;text-align:center;color:#888;padding:40px">Sin resultados para "'+q+'"</div>'; return; }
...
function __imgInsert(url, desc, author) {
  if (typeof insertHTML==='function') {
    insertHTML(`<figure style="margin:12px 0;text-align:center"><img src="${url}" alt="${desc}" style="max-width:100%;border-radius:6px;box-shadow:0 2px 8px rgba(0,0,0,.15)"><figcaption style="font-size:9pt;color:#888;margin-top:4px">${desc}${author?' · Foto: '+author:''} · Libre de derechos</figcaption></figure>`);
  }
  document.getElementById('__imgSearchPanel').remove();
  showToast('🖼️ Imagen insertada ✅');
}


/* ═══════════════════════════════════
   FEATURE 4: GENERADOR DE CV CR
   ═══════════════════════════════════ */
...
function openCVGenerator() {
  ...
  const estilos = [
    { id:'moderno',     icon:'🎨', name:'Moderno',       color:'#003da5' },
    { id:'clasico',     icon:'📄', name:'Clásico',       color:'#1a2942' },
    { id:'ejecutivo',   icon:'💼', name:'Ejecutivo',     color:'#2c3e50' },
    { id:'creativo',    icon:'✨', name:'Creativo',      color:'#8e44ad' },
  ];
  ...
  <div style="background:#fff;border-radius:20px;max-width:520px;width:100%;max-height:94vh;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,.3)">
    <div style="padding:18px 20px 0;flex-shrink:0">
      <div style="font-size:17px;font-weight:700;color:#1a2942;margin-bottom:4px">👤 Generador de CV · Costa Rica</div>
      <div style="font-size:12px;color:#888;margin-bottom:14px">Currículum vitae profesional formato CR</div>
    ...
    <div style="flex:1;overflow-y:auto;padding:0 20px 16px">
      <div style="display:flex;flex-direction:column;gap:8px">
        ${[
          ['__cvNombre',    'Nombre completo *',         'Juan Carlos Pérez Rodríguez'],
          ['__cvProfesion', 'Profesión / Título *',      'Ingeniero en Sistemas · Lic. en Administración'],
          ['__cvEmail',     'Correo electrónico',        'juan.perez@email.cr'],
          ['__cvTel',       'Teléfono',                  '8888-1234'],
          ['__cvDireccion', 'Dirección',                 'San José, Costa Rica'],
          ['__cvLinkedin',  'LinkedIn / Portfolio',      'linkedin.com/in/juanperez'],
          ['__cvPerfil',    'Perfil profesional',        'Profesional con X años de experiencia en...'],
          ['__cvExp',       'Experiencia (empresa | cargo | años | descripción, separados por ;)', 'Empresa ABC | Desarrollador Senior | 2020-2024 | Desarrollo de aplicaciones web'],
          ['__cvEduc',      'Educación (institución | título | año, separados por ;)', 'UCR | Ingeniería en Sistemas | 2019;TEC | Maestría en IA | 2022'],
          ['__cvHabilidades','Habilidades (separadas por coma)', 'JavaScript, React, Node.js, SQL, Inglés C1, Trabajo en equipo'],
          ['__cvIdiomas',   'Idiomas', 'Español (nativo), Inglés (avanzado), Portugués (básico)'],
          ['__cvReferencias','Referencias', 'Disponibles a solicitud'],
        ].map(([id,label,ph])=>`
          <div>
    ...
    <div style="padding:12px 20px 16px;border-top:1px solid #eee;display:flex;gap:8px;flex-shrink:0">
      ...
      <button onclick="__generateCV()" style="flex:2;padding:10px;border:none;background:#1a2942;color:#fff;border-radius:10px;cursor:pointer;font-size:14px;font-weight:600">👤 Generar CV</button>
    </div>
  </div>`;

  document.body.appendChild(ov);
  ov.addEventListener('click', e=>{if(e.target===ov)ov.remove();});
  window.__cvStyle = 'moderno';
}

function __generateCV() {
  const g    = id => document.getElementById(id)?.value?.trim()||'';
  const nombre    = g('__cvNombre');
  const profesion = g('__cvProfesion');
  const email     = g('__cvEmail');
  const tel       = g('__cvTel');
  const dir       = g('__cvDireccion');
  const linkedin  = g('__cvLinkedin');
  const perfil    = g('__cvPerfil');
  const expRaw    = g('__cvExp').split(';').filter(e=>e.trim());
  const educRaw   = g('__cvEduc').split(';').filter(e=>e.trim());
  const habilidades= g('__cvHabilidades').split(',').map(h=>h.trim()).filter(h=>h);
  const idiomas   = g('__cvIdiomas');
  const refs      = g('__cvReferencias')||'Disponibles a solicitud';
  const style     = window.__cvStyle || 'moderno';

  if (!nombre||!profesion) { showToast('Completá nombre y profesión'); return; }

  const colors = { moderno:'#003da5', clasico:'#1a2942', ejecutivo:'#2c3e50', creativo:'#8e44ad' };
  const color  = colors[style] || '#003da5';

  const expHTML = expRaw.map(e => {
    const [empresa,cargo,periodo,desc] = e.split('|').map(s=>s.trim());
    return `<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between"><strong style="color:#1a1a1a">${cargo||e}</strong><span style="font-size:10pt;color:#888">${periodo||''}</span></div>${empresa?`<div style="font-size:10pt;color:${color};font-weight:600">${empresa}</div>`:''}<div style="font-size:10pt;color:#555;margin-top:2px">${desc||''}</div></div>`;
  }).join('');

  const educHTML = educRaw.map(e => {
    const [inst,titulo,anio] = e.split('|').map(s=>s.trim());
    return `<div style="margin-bottom:8px"><div style="display:flex;justify-content:space-between"><strong style="color:#1a1a1a">${titulo||e}</strong><span style="font-size:10pt;color:#888">${anio||''}</span></div>${inst?`<div style="font-size:10pt;color:${color}">${inst}</div>`:''}</div>`;
  }).join('');
  ...
  <div style="font-family:Calibri,Arial,sans-serif;max-width:17cm;margin:0 auto">
    ...
    <div style="background:${color};color:#fff;padding:24px 28px;border-radius:8px 8px 0 0;margin-bottom:0">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:12px;margin-top:10px;font-size:9pt;opacity:.85">
        ${email?`<span>✉️ ${email}</span>`:''}${tel?`<span>📱 ${tel}</span>`:''}
        ${dir?`<span>📍 ${dir}</span>`:''}${linkedin?`<span>🔗 ${linkedin}</span>`:''}
      </div>
    </div>
    ...
    <div style="border:1px solid ${color}33;border-top:none;border-radius:0 0 8px 8px;overflow:hidden">
      <div style="display:grid;grid-template-columns:2fr 1fr">
        ...
        <div style="padding:20px 20px 20px 24px;border-right:1px solid #eee">
          ...
          ${educHTML?`<div><div style="font-size:11pt;font-weight:700;color:${color};border-bottom:2px solid ${color};padding-bottom:4px;margin-bottom:10px;text-transform:uppercase;letter-spacing:.5px">Educación</div>${educHTML}</div>`:''}
        </div>
        <!-- Columna lateral -->
    ...
    <div style="text-align:center;font-size:8pt;color:#aaa;margin-top:6px">CV generado con WC Corporate Editor · Costa Rica · ${new Date().getFullYear()}</div>
  </div>`;
...
/* ═══════════════════════════════════
   FEATURE 5: CALCULADORA IVA CR
   ═══════════════════════════════════ */

function openIVACalc() {
  ...
  <div style="background:#fff;border-radius:24px 24px 0 0;width:100%;max-width:560px;padding:20px;box-shadow:0 -8px 40px rgba(0,0,0,.2);font-family:-apple-system,sans-serif">
    ...
    <div style="font-size:12px;color:#888;margin-bottom:16px">Ley N° 9635 · Impuesto al Valor Agregado</div>
    ...
    <div id="__ivaResult" style="background:#f0f7ff;border-radius:12px;padding:14px;margin-bottom:14px;display:none">
      ...
      <button onclick="__ivaInsert()" style="width:100%;padding:8px;border:none;background:#003da5;color:#fff;border-radius:8px;cursor:pointer;font-size:12px;font-weight:600">📋 Insertar desglose en documento</button>
    </div>
    ...
    <div style="margin-bottom:14px">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:11px">
        ${[['13%','Bienes y servicios generales'],['4%','Servicios médicos y medicamentos'],['2%','Canasta básica tributaria'],['1%','Seguros (ISN)'],['0%','Exportaciones y exentos']].map(([t,d],i)=>`
...
function __calcIVA() {
  const monto  = parseFloat(document.getElementById('__ivaMonto')?.value)||0;
  const tarifa = parseInt(document.getElementById('__ivaTarifa')?.value)||13;
  const tipo   = document.querySelector('[name="__ivaTipo"]:checked')?.value||'sinIVA';
  ...
  let base, montoIVA, total;
  if (tipo === 'sinIVA') {
    base     = monto;
    montoIVA = monto * tarifa / 100;
    total    = monto + montoIVA;
  } else {
    total    = monto;
    base     = monto / (1 + tarifa/100);
    montoIVA = total - base;
  }

  document.getElementById('__ivaBase').textContent      = fmt(base);
  document.getElementById('__ivaMontoIVA').textContent  = fmt(montoIVA);
  document.getElementById('__ivaTotal').textContent     = fmt(total);

  window.__ivaData = { base, montoIVA, total, tarifa };
}

function __ivaInsert() {
  const d = window.__ivaData;
  if (!d) return;
  const fmt  = n => '₡' + Math.round(n).toLocaleString('es-CR');
  const html = `<table style="border-collapse:collapse;font-family:Calibri,Arial,sans-serif;font-size:10pt;margin:8px 0;min-width:280px">
    <tr style="background:#003da5;color:#fff"><th colspan="2" style="padding:6px 10px;text-align:left">Desglose IVA · Costa Rica (${d.tarifa}%)</th></tr>
  ...
  showToast('🧮 Desglose IVA insertado ✅');
}
  ...
  setTimeout(function() {
    ...
    if (typeof orig === 'function') {
      window.handleAction = function(a) {
        switch(a) {
          case 'cloudSave':     openCloudSavePanel(); break;
          case 'immersive':     openImmersiveMode();  break;
          case 'freeImages':    openFreeImages();     break;
          case 'cvGenerator':   openCVGenerator();    break;
          case 'ivaCalc':       openIVACalc();        break;
          default: orig(a);
        }
      };
    }
    console.log('WC Corporate Editor v25.35 ✅ — Cloud Save, Inmersiva, Imágenes, CV, IVA CR');
  }, 4300);
})();
